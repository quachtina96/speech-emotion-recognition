% File-specific, General COVAREP feature extraction script
%
% Description
%  This script extracts features to do with glottal source and spectral
%  envelope available with the COVAREP repository. For the .wav file in
%  given, .mat files are produced containing 
% 
% Input
%  wav_file_path [path to wav file] : Path to wav file to be analyzed
...
%         feature matrix: features [number of frames X 74] and names:
%         containing the feature name correspond to each column of the
%         feature matrix
% Example
%   wav_file_path='//home/john/Desktop/test_dir/my_wav.wav'; % Specify directory of wavs
%   sample_rate=0.01; % State feature sampling rate
%   COVAREP_feature_extraction_on_file(wave_file_path,sample_rate); % Launch feature extraction
%
...
%  Tina Quach <quacht@mit.edu> 


function [glottal_context_windows, names, features] = COVAREP_feature_extraction_on_file(wave_file_path,sample_rate, frame_length, frame_shift, num_frames)
...
%% Do processing
slash_occurences = strfind(wave_file_path,'/');
basename = wave_file_path(slash_occurences(end)+1:length(wave_file_path)-4);
in_dir = wave_file_path(1:slash_occurences(end));
disp(['Analysing file: ' basename])
try
    % Load file and set sample locations
    [x,fs]=audioread(wave_file_path);
    feature_sampling=round((sample_rate/2)*fs):round(sample_rate*fs):length(x);
    feature_sampling=feature_sampling(1:num_frames);
    size_feat = size(feature_sampling)
    ...
    % Create context_windows from the glottal source waveform.
    glottal_context_windows = get_context_windows(g_iaif', frame_length, frame_shift);
